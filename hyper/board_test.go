...
func TestBoard_NextStop(t *testing.T) {
	...
	for _, testcase := range testcases {
		t.Run(testcase.Name, func(t *testing.T) {
			board, err := hyper.NewBoard(size, hyper.Placement{Actor: hyper.RandomPlace, Goal: hyper.RandomPlaceNearByWalls})
			if err != nil {
				t.Fatal(err)
			}
			board.NewGame()
...
func TestBoard_MoveActor(t *testing.T) {
	...
	for _, testcase := range testcases {
		t.Run(testcase.Name, func(t *testing.T) {
			board, err := hyper.NewBoard(hyper.Size{16, 16}, hyper.Placement{Actor: hyper.RandomPlace, Goal: hyper.RandomPlaceNearByWalls})
			if err != nil {
				t.Fatal(err)
			}
			board.NewGame()
